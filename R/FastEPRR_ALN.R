...
FastEPRR_ALN <- function(alnFilePath=NULL, format=-1, erStart=NULL, erEnd=NULL, winLength=NULL, stepLength=NULL, demoParameter=NULL, outputFilePath=NULL) {
 ...
 # dyn.load("FastEPRR.dll")
  ...
  assign("FastEPRRDataInfos", data.frame(nsam=integer(), wdou=integer(), wxton=integer(), wH=integer(), wHetero=numeric(), wavsk2=numeric(), wavr2=numeric(), misInfo=character(), startPos=integer(), endPos=integer(), rho=numeric(), rhoL=numeric(), rhoR=numeric(), flag=integer(), stringsAsFactors=FALSE), envir = .GlobalEnv)
